document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- STATE & ASSETS ---
    let state = { activeTool: 'brush', image: null, imageContainer: null, layers: [], history: [], historyIndex: -1, activeLayerId: null, isDrawing: false, isMaskDrawn: false, zoomLevel: 1, apiKey: null };
    const PROMPT_DATA = {
    ...
    function handleCategoryChange() {
        const namesContainer = getEl('object-names-container');
        namesContainer.innerHTML = '';
        const categoryType = getEl('category-type').dataset.value;

        if (categoryType && PROMPT_DATA.categories[categoryType]) {
            const headingText = categoryType.charAt(0).toUpperCase() + categoryType.slice(1);
            createHeading(namesContainer, 'fa-pen-to-square', `Edit Object Names (${headingText})`);
            PROMPT_DATA.categories[categoryType].forEach(control => {
                createCustomDropdown(control, updateGenerateButtonState, namesContainer, `Select ${control.label}...`);
            });
        }
        updateGenerateButtonState();
    }
    ...
    async function handleEmptyRoom() {
        state.isMaskDrawn = true; // Mark as if a mask was drawn
        await handleGeneration("remove all furniture and objects from the room, creating a clean, empty room with realistic walls, floor, and ceiling");
        document.querySelector('.empty-room-btn')?.remove();
    }
    async function handleGeneration(overridePrompt = null) {
        ...
        showLoader(async () => {
            try {
                ...
                const mode = overridePrompt ? 'empty' : promptTabs.querySelector('.active').dataset.mode;
                ...
                const layersToComposite = (mode === 'change' || mode === 'remove') && state.activeLayerId !== 0
                    ? state.layers.filter(l => l.id !== state.activeLayerId && l.id !== 0)
                    : state.layers.filter(l => l.id === 0);
                
                compositeCtx.drawImage(state.layers.find(l => l.id === 0).canvas, 0, 0); // Always start with base
                ...
                if (overridePrompt) { // For empty room, mask is the whole image
